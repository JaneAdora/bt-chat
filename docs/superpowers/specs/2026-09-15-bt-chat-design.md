...
- Preamble: before the first frame on a new socket, each side sends its 8-byte node id (see 4.1). A second implementation, such as the v1.5 Linux client, must send and read this preamble.
...
- Status label under outbound messages: waiting, sent, delivered, read. Words rather than glyphs, for outdoor readability.
...
- Tie-break rule is a pure function with a table test. Keepalive is a pure state machine with a table test.
...
- Tie-break by a random node id exchanged in a preamble, not by adapter address: Android 14 returns a placeholder address to apps, so the address rule cannot work.
